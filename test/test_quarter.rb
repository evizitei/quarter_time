...
class TestQuarter < Test::Unit::TestCase
  context "Quarter" do
    ...
    context '.previous' do
      should "build a new quarter object for the previous quarter during the majority of the year" do
        begin
          Timecop.travel(Date.civil(2013, 12))
          quarter = Quarter.previous
          assert_equal 2013, quarter.year
          assert_equal 3, quarter.quarter
        ensure
          Timecop.return
        end
      end

      should "build a new quarter object for the previous year and quarter during the first quarter" do
        begin
          Timecop.travel(Date.civil(2013))
          quarter = Quarter.previous
          assert_equal 2012, quarter.year
          assert_equal 4, quarter.quarter
        ensure
          Timecop.return
        end
      end
